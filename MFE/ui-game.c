...
/* Forward declare this function as it's used in the command lists */
...
void do_cmd_monster_browser(void)
{
	...
	if (result != 0) {
		msg("Failed to execute monster browser script.");
		message_flush(EVENT_MESSAGE, NULL, NULL);
	}
	
	/* Ensure the character screen gets redrawn properly */
	do_cmd_redraw();
}